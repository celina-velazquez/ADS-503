...
head(train_data)
```
To create the training and test sets, instead of a stratified random sample or a simple random sample, a time-ordered sample was taken so that future observations did not leak into the training set given the time-series nature of influenza data. Next, the predictor set was created by removing all four predictors. The target variable, target_1wk was identified. Lastly, cross-validation folds were created.
```{r, cache = TRUE}
# Fit the Model
# Ridge Regression
